# This is a duplication/renaming of the following lib: https://hex.pm/packages/aho_corasick
# This is done because of a naming clash with the erlang library :aho_corasick. This lib is only used for benchmark comparisons
defmodule ExAhoCorasick do
  @moduledoc """
  Usage:
  ```elixir
  graph = ExAhoCorasick.new(["my", "dictionary", "terms"])
  results = ExAhoCorasick.search(graph, "I wonder if any of the terms from my dictionary appear in this text, and if so, where?")
  ...
  @doc """
  Create a new ExAhoCorasick graph, but don't populate it. You will need to
  call `add_token` to add tokens, and then `build_trie` before searching
  against this ExAhoCorasick
  """
  def new() do
    g = :digraph.new()
    :digraph.add_vertex(g, :root)
    %ExAhoCorasick{graph: g}
  end

  @doc """
  Create a new fully-formed ExAhoCorasick graph. Pass in all the dictionary terms
  ...
  ```elixir
  g = ExAhoCorasick.new()
  ExAhoCorasick.add_term(g, "a term")
  # must be called before calling search!
  ExAhoCorasick.build_trie(g)
  # internal trie/graph is built. now you can search:
  ExAhoCorasick.search(g, input_text)
  ```
  """
